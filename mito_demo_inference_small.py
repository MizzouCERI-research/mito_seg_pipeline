...
patch_data = get_data(data_root='data/demo', batch_size=(1,1,1,1), num_patches=(4,4,0,0), patch_size=(patch_size_train, patch_size_val, patch_size_test))
...
image_file = "./data/demo/rat/images/mitoem_r_0400_0415_image_validation_1_320.mat"
# %%
with torch.no_grad():
    iterloader = iter(eval_loader)
    batch_data = next(iterloader)
    # batch_image = batch_data['image']
    batch_target = batch_data['label']
    batch_file_name = batch_data['file_name']
         
    batch_image = hdf5storage.loadmat(image_file)
    batch_image = batch_image['image']

    im_min = np.min(batch_image)
    im_max = np.max(batch_image)
    batch_image = (batch_image - im_min) / (im_max - im_min)
